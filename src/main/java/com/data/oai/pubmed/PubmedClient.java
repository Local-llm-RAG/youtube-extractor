...
public class PubmedClient {
    ...
    public byte[] listRecords(String baseUrl, String from, String until,
        ...
        URI uri = HttpExchangeSupport.buildListRecordsUri(baseUrl, from, until, token, metadataPrefix, set);
        // PMC returns 404 when no records exist for a date range — treat as null body
        return HttpExchangeSupport.executeExchange(rest, uri, code -> code == 404, uri.toString());
    }
    ...
    public byte[] fetchOaLinks(String pmcId) {
        ...
        return HttpExchangeSupport.executeExchange(rest, uri, code -> false, pmcId);
    }
