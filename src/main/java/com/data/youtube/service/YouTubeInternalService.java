package com.data.youtube.service;

import com.google.api.services.youtube.model.*;
import com.data.youtube.persistence.entity.ChannelEntity;
import com.data.youtube.persistence.entity.Video;
import com.data.youtube.persistence.repository.ChannelRepository;
import com.data.youtube.persistence.repository.VideoRepository;
import com.data.youtube.persistence.repository.YouTubeRegionLanguageRepository;
...
public class YouTubeInternalService {
    ...
    @Transactional
    public ChannelEntity upsertChannelFromYouTube(Channel ytChannel) {
        ChannelSnippet snippet = ytChannel.getSnippet();

        ChannelEntity incoming = buildFromYoutubeChannel(ytChannel, snippet);
    ...
    private Video buildVideoDao(com.google.api.services.youtube.model.Video video, ChannelEntity channel, Map<String, Map<String, String>> categoryMap) {

        Video videoDao = videoRepository.findByYoutubeVideoId(video.getId())
                .orElseGet(Video::new);

        videoDao.setYoutubeVideoId(video.getId());
        videoDao.setChannel(channel);
    ...
    private static ChannelEntity buildFromYoutubeChannel(Channel ytChannel, ChannelSnippet snippet) {
